...
def get_batch(split='train'):
    ...
    x = torch.stack([torch.from_numpy((data[i:i+block_size]).astype(numpy.int64)) for i in ix])
    y = torch.stack([torch.from_numpy((data[i+1:i+1+block_size]).astype(numpy.int64)) for i in ix])
